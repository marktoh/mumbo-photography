...
function Banner() {
  return <div className="banner">Free shipping is now available</div>;
}
function PhotoView() {
  const { id } = useParams();
  ...
  function Prices({ id, size, currency, price, onClick, isHighlighted }) {
    return (
      <button
        className={`pricing-option${isHighlighted ? " highlight" : ""}`}
        onClick={() => onClick(id)}
      >
    ...
    <div className="photo-view">
      ...
      <div className="photo-details">
        <div className="description">
          ...
          <p>
            {prices.length > 0 &&
              prices?.find((e) => e.id === currentPriceId)?.nickname}
          </p>
        </div>
              ...
              prices
                ...
                ?.map((price) => (
                  <Prices
                    ...
                    price={formatStripePrice(price.unit_amount)}
                    onClick={handlePricingOptionClick}
                    isHighlighted={currentPriceId === price.id}
                  />
                ))}
          </div>
          <form
            action={`${process.env.REACT_APP_SERVER_URL}/api/create-checkout-session`}
            method="POST"
          >
